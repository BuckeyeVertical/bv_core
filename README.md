...
- **test_servo** — Bench tool for the payload servos in µs: `test_servo show`, `test_servo plate 1685`, `test_servo rest`, `test_servo drop bottle`. See `docs/HITL/payload.md`.
...
area in acres, and includes a street-map layer. It draws the exact snake route
generated by `bv_core.scan_plan`, including direction arrows and the row count.
Use **Long way** or **Short way** to select the row axis, and **Top → bottom** or
**Bottom → top** to select the starting side. The copied YAML includes the
corresponding `scan_sweep` and `scan_start` values. **Use Region** writes those
settings and the displayed `scan_boundary` directly to the active mission YAML.

The **Lap route** tab displays numbered lap waypoints and arrows in flight order.
Choose clockwise or counterclockwise and use **Use Region** to replace the active
...
verify flight boundaries against official or surveyed coordinates.
